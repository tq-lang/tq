...
// streamMemoryTest is a shared helper for TestStreamMemory and TestStreamMemoryTOON.
// It pipes data generated by writeFn into tq --stream and checks peak RSS.
func streamMemoryTest(t *testing.T, label string, writeFn func(io.Writer) error) {
	t.Helper()

	const rssLimit = 50 * 1024 * 1024 // 50 MB peak RSS ceiling
	...
	t.Logf("peak RSS: %.1f MB (limit: %.0f MB, %s)", peakMB, limitMB, label)

	if peak > rssLimit {
		t.Errorf("peak RSS %.1f MB exceeds limit %.0f MB — %s stream mode may be materializing input",
			peakMB, limitMB, label)
	}
}

// TestStreamMemory launches tq --stream with ~100MB of nested JSON piped to
// stdin and checks peak RSS after the process exits.
func TestStreamMemory(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping stream memory test in short mode")
	}
	const elements = 550_000
	streamMemoryTest(t, "JSON ~100 MB", func(w io.Writer) error {
		return nestedArrayWriter(w, elements)
	})
}
...
func TestStreamMemoryTOON(t *testing.T) {
	...
	const elements = 550_000
	streamMemoryTest(t, "TOON ~100 MB", func(w io.Writer) error {
		return toonNestedArrayWriter(w, elements)
	})
}
