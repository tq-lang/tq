...
func TestWrite(t *testing.T) {
	tests := []struct {
		...
		wantSub  string // substring match if non-empty
		wantErr  bool
	}{
		// Raw output
		...
		// TOON output — assert actual content
		{"toon default", map[string]any{"x": "y"}, Options{}, "x: y\n", "", false},
		{"toon pipe delimiter", []any{1.0, 2.0}, Options{Delimiter: toon.DelimiterPipe}, "", "|", false},
		{"toon tab delimiter", []any{"a", "b"}, Options{Delimiter: toon.DelimiterTab}, "", "\t", false},
		{"toon custom indent", map[string]any{"x": "y"}, Options{Indent: 4}, "", "x: y", false},
		{"toon tab indent", map[string]any{"x": "y"}, Options{Tab: true}, "", "x: y", false},
